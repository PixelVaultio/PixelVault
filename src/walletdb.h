...
public:
    ...
    bool WriteZPXLSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZPXLSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZPXLSeed_deprecated(uint256& seed);
    bool EraseZPXLSeed();
    bool EraseZPXLSeed_deprecated();

    bool WriteZPXLCount(const uint32_t& nCount);
    bool ReadZPXLCount(uint32_t& nCount);
