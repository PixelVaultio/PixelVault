...
CzPXLWallet::CzPXLWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zpxl seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPXLSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPXLSeed_deprecated()) {
                    LogPrintf("%s: Updated zPXL seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zpxl seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zpxl wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzPXLWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPXLCount(nCountLastUsed);
    else if (!walletdb.ReadZPXLCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPXLWallet::Lock()
{
    seedMaster = 0;
}

void CzPXLWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPXLWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPXL(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPXLWallet::LoadMintPoolFromDB()
{
...
void CzPXLWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPXLWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPXLWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zpxlTracker->HasPubcoinHash(pMint.first)) {
...
bool CzPXLWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPXL(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zpxlTracker which also adds to database
    pwalletMain->zpxlTracker->Add(dMint, true);
...
void CzPXLWallet::SeedToZPXL(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzPXLWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPXLWallet::GenerateDeterministicZPXL(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPXLWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPXLWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
