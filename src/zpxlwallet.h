...
#ifndef PIXEL_ZPXLWALLET_H
#define PIXEL_ZPXLWALLET_H
...
public:
    CzPXLWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZPXL(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZPXL(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //PIXEL_ZPXLWALLET_H
