...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered pixel address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 PXL.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal pixel:URI (pixel:XyZ...?message=example)
            {
...
static void NotifyzPXLReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzPXLReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzPXLReset.connect(boost::bind(NotifyzPXLReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzPXLReset.disconnect(boost::bind(NotifyzPXLReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zpxlTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
