...
#include "ui_zpxlcontroldialog.h"
...
std::set<std::string> ZPxlControlDialog::setSelectedMints;
std::set<CMintMeta> ZPxlControlDialog::setMints;

ZPxlControlDialog::ZPxlControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZPxlControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZPxlControlDialog::~ZPxlControlDialog()
{
    delete ui;
}

void ZPxlControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZPxlControlDialog::updateList()
{
...
void ZPxlControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZPxlControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZPxl_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZPxlControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZPxlControlDialog::GetSelectedMints()
{
...
void ZPxlControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
