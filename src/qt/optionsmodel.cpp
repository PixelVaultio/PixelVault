...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::PXL);
    ...
    if (!settings.contains("nAnonymizePixelAmount"))
        settings.setValue("nAnonymizePixelAmount", 1000);

    nAnonymizePixelAmount = settings.value("nAnonymizePixelAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizePixelAmount"))
        SoftSetArg("-anonymizepixelamount", settings.value("nAnonymizePixelAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in pixel.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizePixelAmount:
            return QVariant(nAnonymizePixelAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizePixelAmount:
            nAnonymizePixelAmount = value.toInt();
            settings.setValue("nAnonymizePixelAmount", nAnonymizePixelAmount);
            emit anonymizePixelAmountChanged(nAnonymizePixelAmount);
            break;
        case CoinControlFeatures:
