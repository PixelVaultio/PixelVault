...
CzPXLTracker::CzPXLTracker(std::string strWalletFile)
{
...
CzPXLTracker::~CzPXLTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzPXLTracker::Init()
{
...
bool CzPXLTracker::Archive(CMintMeta& meta)
{
...
bool CzPXLTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzPXLTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzPXLTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzPXLTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzPXLTracker::GetSerialHashes()
{
...
CAmount CzPXLTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzPXLTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzPXLTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzPXLTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzPXLTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzPXLTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzPXLTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzPXLTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzPXLTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzPXLTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzPXLTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzPXLTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzPXLTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzPXLTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzPXLTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzPXLTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzPXLTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzpxl from DB\n", __func__, listDeterministicDB.size());
    }
