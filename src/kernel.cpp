...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZPxlStake(spend));
    } else {
        ...
        CPxlStake* pxlInput = new CPxlStake();
        pxlInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(pxlInput);
    }
