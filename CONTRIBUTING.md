...
The Pixel Vault project operates an open contributor model where anyone is
...
The following applies to code changes to the Pixel Vault project, and is not to be
confused with overall Pixel Network Protocol consensus changes.

Whether a pull request is merged into Pixel Vault rests with the project merge
...
Patches that change Pixel consensus rules are considerably more involved than
...
Where a patch set proposes to change the Pixel consensus, it must have been
...
The project leader is the release manager for each Pixel Vault release.
